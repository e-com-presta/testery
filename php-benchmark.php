...
// Show or hide server name and IP address
...
function test_benchmark(array $settings)
{
    ...
    if ($databaseTest && isset($settings['db.host']) && isset($settings['db.user']) && isset($settings['db.name'])) {
...
function print_html_result(array $data, bool $showServerName = true)
{
    ...
    $result .= '<tr class="even"><td>Calculation total</td><td>'.h($data['benchmark']['calculation']).'</td></tr>';
    $result .= '</tbody>';
